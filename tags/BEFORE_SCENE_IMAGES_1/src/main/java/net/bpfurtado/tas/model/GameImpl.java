...
public class GameImpl implements Game
{
	...
	private void execCode(Scene scene, List<PostCodeExecutionAction> actions)
	{
		...
		/*
		for (GoToSceneListener goToSceneListener : listeners) {
			//Goto at the moment not being so complete as the
			//OpenPath at the Runner Class.
			//111 Problem Here!!!
	...
	public void execPostCodeActions()
	{
		//OPEN SCENE: GOTOs
		/*if (sceneIdToOpen != -1) {
			openPath(adventure.getScene(sceneIdToOpen));
			sceneIdToOpen = -1;
		}*/
	...
	{
		List<PostCodeExecutionAction> actions = 
			new CodeExecutionAnalyser().analyseCode(this, adventure.getAssertions(), getCurrentScene().getText());
